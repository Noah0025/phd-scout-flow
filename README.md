...
- **C 级** = 降权信号（默认命中 C → 降到 C 评级，仍写入 Notion 让用户判；profile 可设严格排除模式，但默认不主动推开机会）。

评估时用**项目角度覆盖率**：LLM 从 PhD 描述抽 `job_signals`（3-8 个能力点），看 用户关键词库覆盖了多少 = `match_rate`。
分母是项目需求，**不是** 用户关键词库总数（库越大越被稀释是错的视角）。
